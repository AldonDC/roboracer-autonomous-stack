...
def launch_setup(context, *args, **kwargs):
    ...
    csi_nodes = LaunchConfiguration('csi_nodes').perform(context)
    csi_list = [node.strip() for node in csi_nodes.split(',') if (node.strip() and node.strip() in _csi_nodes)]
    rgbd_nodes = LaunchConfiguration('rgbd_nodes').perform(context)
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument('nodes', default_value='command,qcar,csi,rgbd,lidar_qos,imu_external', description='List of nodes to launch'),
        DeclareLaunchArgument('csi_nodes', default_value='csi_right,csi_left,csi_front,csi_back', description='CSI nodes'),
        DeclareLaunchArgument('rgbd_nodes', default_value='rgbd_color,rgbd_depth', description='RGBD nodes'),
        #*launch_nodes
