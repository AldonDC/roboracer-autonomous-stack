...
class LIDARNode(Node):
	...
	def lidar_callback(self):
		...
		#self.get_logger().info("Ready to process...")
	...
	def filter_lidar_data(self, currentScan, currentAngles):
		...
		#self.get_logger().info(f"{anglesNotZero}, {currentAngles}")
		if anglesNotZero > 0:
			...
			checkedAngles = currentAngles>6.5
	...
	def process_lidar_data(self, distances, scan_times):
		...
		#self.get_logger().info(f"Ready to publish /qcar/scan. Time: {scan.scan_time} min_angle:{scan.angle_min}")
